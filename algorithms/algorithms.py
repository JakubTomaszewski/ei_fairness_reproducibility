            ...
            if fairness == 'EI' and torch.sum(Yhat<tau)>0:
                ...
                for i, sensitive_attr in enumerate(sensitive_attrs):
                    for z in sensitive_attr:
                        ...
                        if group_idx.sum() == 0:
                            continue
                        loss_z[z] = loss_func(Yhat_max.reshape(-1)[group_idx], torch.ones(group_idx.sum(), device=device))
                ...
                for i, sensitive_attr in enumerate(sensitive_attrs):
                    for z in sensitive_attr:
                        ...
                        if group_idx.sum() == 0:
                            continue
                        loss_z[z] = (z_batch_e[group_idx, i].shape[0]/z_batch[z_batch[:, i]==z, i].shape[0])*loss_func(Yhat_max.reshape(-1)[group_idx], torch.ones(group_idx.sum(), device=device))
                        f_loss += torch.abs(loss_z[z] - loss_mean)
